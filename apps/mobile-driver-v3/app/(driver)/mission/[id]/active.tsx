...
import { missionMutations, missionProgressMutations, missionQueries, missionService, reportError } from '@taxilink/services';
...
import type { CourseState } from '@/lib/trackingConfig';
...
export default function ActiveCourseRoute() {
  ...
  // Pousse le courseState courant dans le store global → useDriverOnlineTracking
  // (monte dans _layout) bascule de 'idle' (15s/10m) vers 'assigned' (12s/10m
  // balanced) ou 'in_progress' (5s/10m balanced). Au demontage de l'ecran on
  // remet 'idle' pour eviter de continuer a tracker en mode course une fois
  // la course terminee.
  const setCourseState = useDriverOnlineStore((s) => s.setCourseState);
  useEffect(() => {
    if (!step) return;
    const courseState: CourseState =
      step === 'enroute' || step === 'arrived_pickup' ? 'assigned'
      : step === 'onboard' || step === 'arrived_dest' || step === 'dropped' ? 'in_progress'
      : 'idle';
    setCourseState(courseState);
  }, [step, setCourseState]);
  useEffect(() => {
    return () => { setCourseState('idle'); };
  }, [setCourseState]);
  ...
  // Geofencing auto-advance SUPPRIME (2026-05-23) : user reportait des faux
  // positifs en livrant un autre patient pres du pickup. Maintenant que les
  // clics manuels. Notifications locales (start+end reminders) rattrapent les
  // oublis sans nécessiter de GPS.
    ...
    try {
      switch (step) {
        case 'enroute':
          ...
          // Course a vraiment demarre → annule notif start, planifie notif end.
        ...
        case 'dropped':
          await missionMutations.complete(mission.id);
          // Course terminee → cancel toutes les notifs restantes pour cette mission.
          void cancelMissionRemindersForMission(mission.id);
          router.back();
          return;
        case 'done': router.back(); return;
      }
      const refreshed = await missionQueries.getByIdMasked(mission.id);
  ...
  return (
    <SafeAreaView edges={['top', 'bottom']} style={{ flex: 1, backgroundColor: colors.bg }}>
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12, paddingHorizontal: 16, paddingVertical: 12 }}>
        ...
        {/* ACT-03 : actions "Client absent" / "Annuler" accessibles depuis l'ecran
            de course (et plus seulement l'onglet Mes courses). Masque a l'etat done. */}
        {step !== 'done' && (
          <View style={{ width: 40, height: 40, borderRadius: 20, overflow: 'hidden', backgroundColor: colors.surfaceMuted }}>
            ...
            >
              <Text style={{ fontSize: 20, fontWeight: '800', color: colors.ink, lineHeight: 20 }}>···</Text>
            </Pressable>
          </View>
        )}
      </View>
      ...
      <CourseActionsMenu
        ...
        onNoShow={async (m, reason) => {
          try {
            ...
            reportError(err, { tags: { phase: 'active-no-show' } });
            Alert.alert('Action impossible', err instanceof Error ? err.message : 'Réessaie dans un instant.');
          }
        }}
        onCancel={async (m, reason) => {
          try {
            ...
            reportError(err, { tags: { phase: 'active-cancel' } });
            Alert.alert('Annulation impossible', err instanceof Error ? err.message : 'Réessaie dans un instant.');
          }
        }}
      />
    </SafeAreaView>
