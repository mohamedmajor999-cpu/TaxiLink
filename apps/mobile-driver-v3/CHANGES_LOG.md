# CHANGES LOG — apps/mobile-driver-v3
...
**Origine v3** : fork de `apps/mobile-driver-v2` le 2026-05-15 pour appliquer le
redesign V7 (BottomNav permanente + Mapbox natif + dark mode Tesla). L'historique
ci-dessous #1 → #N reprend les entrées v2 (la v3 a hérité de tout).

---

## V7-redesign — Refonte navigation + carte (2026-05-15 → en cours)

**Nouveautés v3 par rapport a v2** :
- `src/components/navigation/BottomNav.tsx` — barre de navigation fixe (Carte / Courses / + FAB / Groupes / Profil) avec auto-hide sur les routes plein-ecran.
- `src/components/missions/MissionMapMapbox.tsx` — carte SDK natif `@rnmapbox/maps` qui remplace la WebView Leaflet de v2 pour la home (pin pricing 3D, pitch, satellite/streets/dark).
- `src/lib/theme.tsx` — `ThemeProvider` + `nightModeStore` persistant + `Appearance` + NavigationBar Android. Palette Tesla Driver Display.
- `app/(driver)/annonces.tsx` (+354 l.) — ecran "Mes annonces" V7.
- `app/(driver)/courses.tsx` refactorisee (501 → 249 l.) — suppression des tabs, ajout `DateStrip` + `AdsBanner` + `DayCoursesList`.

**Composants v2 conserves** : tout le pipeline GPS (`useDriverGpsTracking`, `useDriverOnlineTracking` V8 HyperSense), realtime (`MissionRealtimeProvider`), push (`usePushRegistration` + 3 channels), notifications in-app, voice parse, geofence.

---

## Fix critique 2026-05-20 — Online cassé par BottomNav + cleanup

**Bug racine** : avec la BottomNav v3, la home se demonte des qu'on tape sur un autre onglet. `useDriverOnlineTracking` et `useDriverHeartbeat` vivaient sur la home → leur cleanup coupait le foregroundService GPS et le ping `last_seen_at` toutes les 30s. La DB marquait le chauffeur `is_online=false` en 3 min via le cron `offline-after` → cascade d'offres ne le servait plus.

**Fix** :
- Ajout : [src/lib/driverOnlineStore.ts](src/lib/driverOnlineStore.ts) — store zustand global pour `isOnline` + `courseState`.
- Modifie : [app/(driver)/_layout.tsx](app/(driver)/_layout.tsx) — `useDriverOnlineTracking` + `useDriverHeartbeat` hisses dans `DriverNotifiersBridge`, plus hydration `isOnline` depuis la DB au mount.
- Modifie : [app/(driver)/index.tsx](app/(driver)/index.tsx) — retrait des hooks, lecture du store global pour le toggle UI.
- Modifie : [app/(driver)/mission/[id]/active.tsx](app/(driver)/mission/[id]/active.tsx) — ecrit `courseState` (`assigned` / `in_progress`) dans le store selon le step geofence ; reset `idle` au demontage.

**Fix collatéraux GPS** :
- Modifie : [src/lib/init.ts](src/lib/init.ts) — `useGpsStore.persist.rehydrate()` au boot pour eviter le cold-start vide.
- Supprime : `app/(driver)/demo-mapbox.tsx` + lien `SideBarDrawer.tsx:225` — la route demo activait `<UserLocation />` Mapbox en parallele du foregroundService expo-location (2 radios GPS, jitter, drain).

**Sécurité Mapbox** :
- Ajout : [app.config.js](app.config.js) — config dynamique qui injecte `RNMapboxMapsDownloadToken` depuis `process.env.RNMAPBOX_DOWNLOAD_TOKEN`.
- Modifie : [app.json](app.json) — retrait du `sk.*` committé en clair.
- ⚠️ TODO user : revoquer l'ancien token sur mapbox.com + `npx eas secret:create --name RNMAPBOX_DOWNLOAD_TOKEN`.

**APK size** :
- Modifie : [eas.json](eas.json) — profile preview build arm64 only (`REACT_NATIVE_ARCHITECTURES=arm64-v8a`) → -30 a -40 MB au prochain build.
- Modifie : [package.json](package.json) — drop `react-native-web` + `react-dom` (inutilisees) → -1 a -2 MB bundle.

**Cleanup dette technique** :
- Supprime 9 composants orphelins missions/ (~1500 lignes mortes) : `MissionMap.tsx` + `leafletHtml.ts` (vestiges Leaflet), `MissionsSheet.tsx` + `MissionSheetItem.tsx`, `MissionMapPopup.tsx`, `DriverHomeAcceptBar.tsx`, `DriverHomeFilterChips.tsx`, `MapControls.tsx`, `PostCourseFab.tsx`.
- Ajout : [src/components/missions/homeFilterTypes.ts](src/components/missions/homeFilterTypes.ts) — extraction du type `HomeTypeFilter` + const `HOME_TYPE_FILTERS` (etait dans DriverHomeFilterChips supprime).
- Fix : [src/components/onboarding/useOnboardingPage.ts](src/components/onboarding/useOnboardingPage.ts) — erreur TS sur `SLIDES[idx+1]` (noUncheckedIndexedAccess) resolue via `?? 'welcome'`.

**Unification carto Mapbox (2026-05-20, soir)** :
- Reecriture : [src/components/active/ActiveMissionMap.tsx](src/components/active/ActiveMissionMap.tsx) — de WebView Leaflet vers `@rnmapbox/maps` natif. Meme moteur que la home, pin dest rouge + pin driver bleu live, auto-fit camera sur les 2 pins a chaque update GPS. Style follow dark/light Tesla.
- Drop : `react-native-webview` du `package.json` (plus aucun consommateur apres la migration Leaflet → Mapbox sur la home et active). Gain APK estime ~1-2 MB + suppression d'une lib native qui faisait son propre process.

**Decoupage des 4 plus gros fichiers (2026-05-20, soir)** :
- [src/components/missions/IncomingMissionOfferModal.tsx](src/components/missions/IncomingMissionOfferModal.tsx) : 519 → **103** lignes. Sous-composants UI extraits dans `IncomingMissionOfferModal.parts.tsx` (PriceRow, TagsRow, TripBlock, PatientRow, NotifHead, PulsingDot, SecondaryButton, PrimaryAcceptButton, CountdownBar, TagPill).
- [app/(driver)/mission/[id].tsx](app/(driver)/mission/[id].tsx) : 693 → **198** lignes. Sous-composants + helpers RGPD extraits dans `src/components/mission-detail/missionDetailParts.tsx` (MaskedNotice, ContactNavActions, HighlightsGrid, RouteCard, DetailsCard, NotesCard, canSeeFullMission, maskName, formatDuration).
- [app/(driver)/group/[id].tsx](app/(driver)/group/[id].tsx) : 795 → **209** lignes. Splits en 3 fichiers : `groupDetailParts.tsx` (Stat, Avatar, MemberRow, ContactBtn, MyStatsPanel, ActivityFeed, memberDisplayName), `groupDetailSections.tsx` (GroupHero, AvailableBanner, OnlineMembersStrip, GroupCtaButtons, MembersSection, GroupDangerButton, GroupConfirmModals), `useGroupDetailData.ts` (fetch + derivations myStats/isAdmin/onlineMembers).
- `src/components/courses/poster/usePosterCourse.ts` (456 l.) : decoupage **non execute**. Le hook regroupe une dizaine de slices de state tres couples (form fields + voice flows + routing + submit + savedDefaults). Toute extraction obligerait a passer des objets de props volumineux entre hooks, fragmentant sans ameliorer la lisibilite. Marque "exception assumee" — a re-evaluer si une feature devient une vraie responsabilite distincte.

**Tests unitaires sur les hooks critiques (2026-05-20, soir)** :
- Setup : [vitest.config.ts](vitest.config.ts) (run en Node, stub `expo-secure-store` via alias) + script `npm test` / `npm run test:watch`.
- **45 tests** repartis sur 3 fichiers :
  - [src/__tests__/trackingConfig.test.ts](src/__tests__/trackingConfig.test.ts) — 21 cas sur `profileFor()` (5 priorites de la fonction de decision GPS : batterie critique, stationary+idle, stationary+assigned, background idle, comportement nominal) + invariants de coherence + `profilesEqual`. C'est le coeur de la consommation batterie : une regression ici tue le tel des chauffeurs en course.
  - [src/__tests__/missionDetailHelpers.test.ts](src/__tests__/missionDetailHelpers.test.ts) — 20 cas sur les helpers RGPD Art. 9 (`canSeeFullMission`, `maskName`, `formatDuration`). Matrice exhaustive {status × role-relation} pour eviter d'exposer du nom patient + telephone + notes a un chauffeur non autorise via URL directe.
  - [src/__tests__/driverOnlineStore.test.ts](src/__tests__/driverOnlineStore.test.ts) — 4 cas sur le store global (setters isOnline + courseState + independance). Couvre la regression principale du fix online du jour (BottomNav cassait la session).
- Refactor leger : extraction des helpers purs (`canSeeFullMission`, `maskName`, `formatDuration`) depuis `missionDetailParts.tsx` (qui import RN) vers un fichier dedie [src/components/mission-detail/missionDetailHelpers.ts](src/components/mission-detail/missionDetailHelpers.ts) pour les rendre testables sans stuber `react-native`. Re-export pour preserver les imports existants.
- Run : `npm test` (mobile-driver-v3) → 45 passes en ~300ms.
- **Hooks React eux-memes non testes** : `useDriverOnlineTracking`, `useDriverHeartbeat`, etc. necessiteraient un test renderer RN (jest + react-test-renderer ou react-native-testing-library) — infrastructure plus lourde, ROI faible vs tests des fonctions pures qu'ils orchestrent. A re-evaluer si un bug echappe aux pure-fn tests.

**Suppression du SideBarDrawer (2026-05-20, soir)** :
- La BottomNav V7 etant permanente sur tous les ecrans, le drawer hamburger faisait double emploi. Etait monte uniquement sur `groupes.tsx` et `group/[id].tsx` via `ScreenHamburgerHeader`.
- Ajout : [src/components/navigation/ScreenTopBar.tsx](src/components/navigation/ScreenTopBar.tsx) — top bar minimal (titre + slot droit optionnel, sans hamburger).
- Modifie : [app/(driver)/groupes.tsx](app/(driver)/groupes.tsx) — `ScreenHamburgerHeader` → `ScreenTopBar title="Mes groupes"`.
- Modifie : [app/(driver)/group/[id].tsx](app/(driver)/group/[id].tsx) — 3 occurrences migrees, le titre affiche maintenant le nom du groupe.
- Supprime : `SideBarDrawer.tsx` (336 l.), `ScreenHamburgerHeader.tsx` (83 l.), `src/components/navigation/useDriverDrawer.ts` (106 l.), `src/hooks/useDrawerData.ts` (81 l.), `src/hooks/useDriverDrawer.ts` (123 l.). **~730 lignes de dette V6 eliminees** + suppression du doublon `useDriverDrawer.ts` entre hooks/ et navigation/.
