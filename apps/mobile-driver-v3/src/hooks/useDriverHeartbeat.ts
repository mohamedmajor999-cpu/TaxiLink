...
// Intervalle du heartbeat foreground. Choisi a 60s (au lieu de 30s) :
// independant du fix GPS, garantit que `last_seen_at` reste frais. Le cron
// offline-after a TTL 180s laisse 3 pings de marge — largement suffisant.
// Reduit de moitie les requetes drivers.update par rapport au build precedent
// (cause partielle des paquets WiFi excessifs observes 2026-05-23).
...
export function useDriverHeartbeat({ isOnline, userId }: Options) {
  ...
  useEffect(() => {
    ...
    async function ping() {
      try {
        // Client Supabase partage (singleton injecte par initApp). Avant on
        // recreait un client a chaque tick → 120 nouveaux clients/heure, chaque
        // un parsant AsyncStorage + JWT. Cause de bruit reseau identifiee
        // par l'audit perf du 2026-05-23.
