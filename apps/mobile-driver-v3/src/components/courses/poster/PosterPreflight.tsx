...
// Hauteur de la BottomNav globale (cf. BottomNav.tsx, height: 64 + insets.bottom).
// PosterPreflight doit reserver cette zone en bas pour que le bouton "Continuer"
// ne soit pas masque sous la nav (la nav est en absolute + zIndex 9999).
  ...
  return (
    <View style={{ flex: 1, backgroundColor: colors.bg }}>
      ...
      >
        ...
        {/* Visibilite : visible uniquement si au moins 1 groupe est selectionne.
            Choix entre "Tout le groupe" (defaut, targetUserIds=null) et
            "Personnes choisies" (sous-ensemble de membres). Place juste apres
            "A qui" et AVANT "Type de course" (demande user 2026-05-23). */}
        {c.visibility === 'GROUP' && c.groupIds.length > 0 && (
          <RecipientPicker c={c} />
        )}
      ...
      {/* Footer sticky : checkbox + bouton "Continuer" toujours visibles.
          paddingBottom = hauteur BottomNav + safe area insets pour que le
          bouton soit visible et tapable au-dessus de la barre de navigation. */}
      <View
        style={{
...
function RecipientPicker({ c }: { c: PosterCourseState }) {
  const { colors, isDark } = useTheme();
  const [members, setMembers] = useState<GroupMember[]>([]);
  const [loading, setLoading] = useState(false);
  const [mode, setMode] = useState<'all' | 'pick'>(
    c.targetUserIds && c.targetUserIds.length > 0 ? 'pick' : 'all',
  );
  const firstGroupId = c.groupIds[0] ?? null;

  // Fetch les membres du 1er groupe selectionne. Le ciblage individuel ne
  // s'applique qu'au 1er groupe (UX : si user en selectionne plusieurs, le
  // cas "personnes choisies" est trop ambigu, on revient a "tout les groupes").
  useEffect(() => {
    if (!firstGroupId || c.groupIds.length > 1) {
      setMembers([]);
      setMode('all');
      c.setTargetUserIds(null);
      return;
    }
    let cancelled = false;
    setLoading(true);
    groupService
      .getMembers(firstGroupId)
      .then((rows) => {
        if (cancelled) return;
        setMembers(rows.filter((m) => m.driverId !== c.driverId));
      })
      .catch((err) => reportError(err, { tags: { phase: 'preflight-recipients' } }))
      .finally(() => { if (!cancelled) setLoading(false); });
    return () => { cancelled = true; };
  }, [firstGroupId, c.groupIds.length, c.driverId, c]);

  function toggleMember(id: string) {
    const current = c.targetUserIds ?? [];
    const next = current.includes(id)
      ? current.filter((x) => x !== id)
      : [...current, id];
    c.setTargetUserIds(next.length > 0 ? next : null);
  }
  ...
  const multiGroup = c.groupIds.length > 1;
    ...
    <View style={{ marginBottom: 22 }}>
      ...
      <Grid2>
        <PickCard
          ...
          onPress={setModeAll}
          icon="users"
          title="Tout le groupe"
          sub="MEMBRES"
        />
        <PickCard
          active={mode === 'pick' && !multiGroup}
          onPress={multiGroup ? setModeAll : setModePick}
          icon="user"
          title={multiGroup ? 'Indispo en multi-groupe' : 'Personnes choisies'}
          sub={multiGroup ? '—' : 'COCHE LES MEMBRES'}
        />
      </Grid2>
      {mode === 'pick' && !multiGroup && (
        <View style={{ marginTop: 12 }}>
          {loading ? (
            ...
            <Text style={{ fontSize: 13, color: colors.inkSoft, textAlign: 'center', paddingVertical: 14 }}>
              Aucun autre membre dans ce groupe.
            </Text>
          ) : (
            <View style={{ gap: 6 }}>
              {members.map((m) => {
                const checked = (c.targetUserIds ?? []).includes(m.driverId);
                const initials = (m.fullName ?? '··').split(/\s+/).map((p) => p[0]).filter(Boolean).slice(0, 2).join('').toUpperCase();
                return (
                  <View
                    key={m.driverId}
                    style={{
                      borderRadius: 12,
                      backgroundColor: checked ? (isDark ? 'rgba(59,130,246,0.18)' : 'rgba(255,210,63,0.18)') : colors.surface,
                      borderWidth: 1,
                      borderColor: checked ? (isDark ? colors.accent : '#0F0F0F') : colors.border,
                      overflow: 'hidden',
                    }}
                  >
                    <Pressable
                      onPress={() => toggleMember(m.driverId)}
                    ...
                    >
                      ...
                      >
                        <Text style={{ fontSize: 11, fontWeight: '900', color: colors.ink }}>{initials}</Text>
                      </View>
                      ...
                      <View
                        style={{
                          width: 22, height: 22, borderRadius: 6, borderWidth: 2,
                          borderColor: checked ? (isDark ? colors.accent : '#0F0F0F') : colors.border,
                          backgroundColor: checked ? (isDark ? colors.accent : '#0F0F0F') : 'transparent',
                          alignItems: 'center', justifyContent: 'center',
                        }}
                      >
