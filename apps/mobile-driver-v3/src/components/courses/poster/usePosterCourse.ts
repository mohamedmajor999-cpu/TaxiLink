...
import { defaultDate, defaultTime, buildScheduledAt, parsePrice } from './posterFormUtils';
...
export function usePosterCourse() {
  ...
  // Choix du sous-ensemble de destinataires (V7) fait directement a l'etape 1
  // du Preflight au lieu d'une sheet a la fin. null = tout le groupe (defaut),
  // sinon = liste d'user_ids cibles dans le RPC marketplace.
  const [targetUserIds, setTargetUserIds] = useState<string[] | null>(null);
  ...
  // CPAM : documents que le chauffeur preneur devra recuperer chez le patient.
  // Defaut coches (cas majoritaire). Concatenes aux notes au submit.
  const [pickupBulletin, setPickupBulletin] = useState(true);
  const [pickupBonTransport, setPickupBonTransport] = useState(true);
  ...
  const voice = usePosterVoiceFlow({
    ...
    setPickupBulletin, setPickupBonTransport,
    myGroups,
  });
  ...
  const onSelectPublic = () => {
    setVisibility('PUBLIC');
    setGroupIds([]);
    // En public on ne cible personne en particulier.
  ...
  const toggleGroup = (id: string) => {
    ...
    // Changement de groupe → reset le ciblage individuel (les membres
    // selectionnes appartiennent peut-etre a un autre groupe).
    setTargetUserIds(null);
  };
  ...
  async function submit(opts?: { share?: boolean; targetUserIds?: string[] | null }): Promise<string | null> {
    // Le ciblage individuel est desormais defini a l'etape 1 (Preflight) via
    // setTargetUserIds. opts.targetUserIds reste accepte pour retro-compat
    // mais n'est plus passe par le screen poster-course (qui appelle submit()
    // sans cet override).
    setError(null);
    setSaving(true);
    try {
      ...
      // CPAM : si des documents sont coches, on les prepend aux notes pour
      // que le chauffeur preneur les voie dans le detail mission. Si l'user
      // n'a pas saisi de notes, le bloc documents est seul ; sinon on prefixe.
      let finalNotes = notes.trim();
      if (type === 'CPAM') {
        const docs: string[] = [];
        if (pickupBulletin) docs.push('Bulletin de situation');
        if (pickupBonTransport) docs.push('Bon de transport');
        if (docs.length > 0) {
          const docsLine = `Documents a recuperer : ${docs.join(', ')}.`;
          finalNotes = finalNotes ? `${docsLine}\n${finalNotes}` : docsLine;
        }
      }
      ...
      const payload: MissionInput = {
        ...
        // V7 partage cible : priorite a opts.targetUserIds (retro-compat) puis
        // au state targetUserIds defini a l'etape 1 du Preflight.
        target_user_ids:
          (opts?.targetUserIds && opts.targetUserIds.length > 0)
            ? opts.targetUserIds
  ...
  return {
    ...
    // Ciblage individuel (etape 1) : null = tout le groupe, sinon = liste d'ids
    targetUserIds, setTargetUserIds,
    // form fields
