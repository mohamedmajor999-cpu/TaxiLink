...
// Fire-and-forget : appelle l'edge function notify_drivers_new_mission apres la
// creation d'une mission depuis le mobile. Push dept-wide "Nouvelle course" a
// tous les chauffeurs du departement de la mission qui n'ont pas desactive
// popupNewMission. Cote web, /api/missions l'invoque deja ; le mobile ne le
// faisait PAS (aucun trigger DB ne s'en charge), donc app fermee = chauffeur
// jamais reveille (cf. audit fonctionnel 2026-05-29, constat DISP-02).
//
// L'invoke utilise le JWT de session du chauffeur : la fonction n'exige qu'un
// jeton valide. Best-effort : un echec ne doit PAS invalider la creation.
export function triggerPushNotifyNewMission(missionId: string): void {
  const supabase = getSupabaseClient();
  void supabase.functions
    .invoke('notify_drivers_new_mission', { body: { mission_id: missionId } })
    .then(({ error }) => {
      if (error) console.warn('[pushNotifyTrigger] notify_drivers_new_mission failed', error);
    })
    .catch((err) => console.warn('[pushNotifyTrigger] notify_drivers_new_mission threw', err));
}
