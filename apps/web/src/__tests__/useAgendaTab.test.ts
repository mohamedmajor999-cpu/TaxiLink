...
describe('useAgendaTab — dayPills', () => {
  it('expose 5 pilules de jour', async () => {
    const { result } = renderHook(() => useAgendaTab())
    await waitFor(() => expect(result.current.loading).toBe(false))
    expect(result.current.dayPills).toHaveLength(5)
  })

  it("la première pilule est labellée \"Aujourd'hui\"", async () => {
    const { result } = renderHook(() => useAgendaTab())
    await waitFor(() => expect(result.current.loading).toBe(false))
    expect(result.current.dayPills[0].label).toBe("Aujourd'hui")
  })

  it('la deuxième pilule est labellée "Demain"', async () => {
    const { result } = renderHook(() => useAgendaTab())
    await waitFor(() => expect(result.current.loading).toBe(false))
    expect(result.current.dayPills[1].label).toBe('Demain')
  })
})
...
describe('useAgendaTab — events', () => {
  it('events filtre les missions du jour sélectionné', async () => {
    ...
    const { result } = renderHook(() => useAgendaTab())
    await waitFor(() => expect(result.current.loading).toBe(false))
    // selected = today (2026-05-01), donc seule m1 est dans events
...
// ─── Heures et structure ──────────────────────────────────────────────────────
describe('useAgendaTab — hours et dayHeight', () => {
  it('expose un tableau hours de 8 à 20 inclus', async () => {
    const { result } = renderHook(() => useAgendaTab())
    await waitFor(() => expect(result.current.loading).toBe(false))
  ...
  it('dayHeight est défini', async () => {
    const { result } = renderHook(() => useAgendaTab())
    await waitFor(() => expect(result.current.loading).toBe(false))
    expect(result.current.dayHeight).toBeGreaterThan(0)
  })
})
