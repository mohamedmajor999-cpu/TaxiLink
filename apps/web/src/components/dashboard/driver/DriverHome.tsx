  ...
  return (
    <div className="relative md:flex md:flex-row md:h-screen h-[100dvh] overflow-hidden bg-paper dark:bg-night-bg">
      ...
      <div className={`absolute bottom-0 left-0 right-0 md:relative md:inset-auto md:shrink-0 md:flex md:flex-col md:flex-none md:w-[42%] md:h-full md:border-l md:border-warm-200 z-[600] md:z-auto ${mapFullscreen ? 'hidden md:flex' : ''}`}>
        <div
          ref={sheetRef}
          className="relative shrink-0 md:mt-0 md:!h-auto md:flex-1 md:min-h-0 md:!min-h-0 z-10 bg-transparent md:bg-paper md:dark:bg-night-bg flex flex-col transition-[height] duration-300 ease-out"
          style={{
            height: `${sheetHeightPx}px`,
            // iOS-only lift : env(safe-area-inset-bottom) vaut 0 sur Android et
            // desktop, ~34px sur iPhone X+. Garantit que la handle (40px) ne
            // tombe pas dans la zone home indicator ou iOS bloque les gestes.
            minHeight: 'calc(40px + env(safe-area-inset-bottom))',
          }}
        >
          <DriverHomeSheet
