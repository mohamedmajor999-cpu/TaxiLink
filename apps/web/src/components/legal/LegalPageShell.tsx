...
export function LegalPageShell({ title, lastUpdated, children }: Props) {
  ...
  return (
    <div className="min-h-screen bg-paper">
      ...
      <div className="max-w-7xl mx-auto px-5 md:px-8 py-8 md:py-12 lg:grid lg:grid-cols-[240px_minmax(0,1fr)] lg:gap-12">
        <aside className="hidden lg:block">
          <div className="sticky top-24">
            <p className="text-[11px] font-bold uppercase tracking-wider text-warm-500 mb-3">Documents légaux</p>
            <nav aria-label="Navigation documents légaux" className="flex flex-col gap-1">
              {RELATED.map((r) => {
                ...
                return (
                  <Link
                    key={r.href}
                    href={r.href}
                    className={`flex items-center gap-2 px-3 py-2 rounded-xl text-[13.5px] font-semibold transition-colors ${
                      isActive ? 'bg-warm-100 text-ink' : 'text-warm-600 hover:bg-warm-50 hover:text-ink'
                    }`}
                  >
            ...
            <a href="mailto:support@taxilink.fr" className="mt-4 inline-block text-[12px] text-warm-500 hover:text-ink underline underline-offset-4">
        ...
        <main className="max-w-3xl">
          ...
          <nav aria-label="Documents légaux" className="mt-12 pt-6 border-t border-warm-200 flex flex-wrap gap-3 lg:hidden">
            {RELATED.map((r) => (
