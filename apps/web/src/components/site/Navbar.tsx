      ...
      {open && (
        <div className="md:hidden border-t border-line bg-white px-4 py-4 space-y-2">
          <div className="flex flex-col gap-2">
            <Link href="/auth/login" onClick={close} className="block text-center py-2.5 text-sm font-semibold text-secondary border border-line rounded-xl">
              Se connecter
            </Link>
            <Link href="/auth/register" onClick={close} className="block text-center py-3 text-sm font-bold bg-primary text-secondary rounded-xl">
              Commencer gratuitement
            </Link>
          </div>
        </div>
