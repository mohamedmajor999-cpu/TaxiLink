...
import { formatEur, formatKm, formatTime, formatDate } from '@/lib/utils'
import { MissionTypeBadge } from '@/components/ui/Badge'

export function HistoriqueScreen() {
  const completed = [...mockAgendaRides].reverse()

  const totalRides = completed.length
  const totalEarnings = completed.reduce((s, r) => s + r.priceEur, 0)
