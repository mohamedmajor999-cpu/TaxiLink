// Itineraires : Google Routes API (trafic predit) avec fallback OSRM.
// Cache 7j par heure de depart : un meme trajet rejoue dans la meme heure
// evite un appel Google facture (trafic stable a l'heure).
...
export interface RouteResult {
  distance_km: number
  duration_min: number
  // Duree sans trafic (Google staticDuration) ; null si OSRM ou absente.
...
// 3 decimales ≈ 110m : tolere les micro-variations de geocoding.
function cacheKey(
...
/**
 * Itineraire via Google Routes API avec trafic predit a `departureTime`.
 * Renvoie null en cas d'erreur → fallback OSRM cote appelant.
 */
...
/** Itineraire OSRM (sans trafic), geometrie GeoJSON incluse. Leve si introuvable. */
