...
/**
 * Prereglages que le chauffeur peut memoriser pour ne pas avoir a re-choisir
 * groupe + type de course a chaque creation d'annonce. Stocke dans
 * `auth.users.raw_user_meta_data.mission_defaults`. Pas critique : si la
 * lecture echoue, on retombe sur les valeurs par defaut cote UI.
 */
...
export const userPrefsService = {
  ...
  /**
   * Departements sur lesquels le chauffeur veut recevoir des missions.
  ...
  /**
   * Application GPS preferee du chauffeur ('waze' | 'maps' | 'ask').
   * 'ask' = proposer les deux a chaque navigation (valeur par defaut).
   */
  ...
  /**
   * Groupes mis en favori par le chauffeur. Stocke cote serveur pour survivre
   * aux evictions de localStorage (iOS Safari PWA, nettoyage navigateur).
   */
